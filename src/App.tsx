...
export default function App() {
  ...
  const addTrip = () => {
    ...
    const newTrips = [...trips, {...form, km} as Trip];
    setTrips(newTrips);
    let b=0, p=0;
    newTrips.forEach(t => { if(t.type==='Business') b+=t.km; else p+=t.km; });
    setBiz(b); setPriv(p);
  };
  ...
  const genLB = () => {
    ...
    txt += `\nBusiness ${biz}km Private ${priv}km Total ${biz+priv}km %Business ${biz+priv?Math.round(biz/(biz+priv)*100):0}%\nDRAFT - Keep 5 years`;
    const blob = new Blob([txt], {type:'text/plain'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'SARS_Logbook_DRAFT.txt';
    a.click();
  };

  const calcR7 = () => {
    const trav = Number(r7.trav)||0;
    const subs = Number(r7.subs)||0;
    const days = Number(r7.days)||0;
    const taxTrav = trav*0.2;
    const exempt = Math.min(subs, days*156);
    const taxSubs = subs-exempt;
  ...
  return (
    <div style={{background:'#0a1930', color:'white', minHeight:'100vh', padding:12, fontFamily:'system-ui'}}>
      <div style={{background:'#fffbe6', color:'#000', border:'2px dashed #ff9f1c', padding:12, borderRadius:12, fontSize:12}}>
        <b>💳 DEV RECOVERY - Capitec SL Cengcani 2121467447 - 0847392469</b><br/>Branch 470010 Savings | Ref: Cell+Package | WhatsApp POP 0847392469 → License ILITHA-30D-XXXX<br/>Ilitha Fintech Reg 2026/707498/07 | Post-launch Paystack
      </div>
      <h2 style={{marginTop:14}}>Ilitha Fintech Hub - Logbook IN17 + Rule7 + LegalEstates</h2>
      <p style={{fontSize:12, color:'#a0c4ff'}}>Client generates & submits himself - DRAFT watermarked - No live SARS</p>

      <div style={{background:'white', color:'black', borderRadius:14, padding:14, marginTop:12}}>
        <h3>ZATax + Logbook IN17 + Rule7</h3>
      ...
      <div style={{background:'#fff8e6',color:'black',borderRadius:14,padding:14,marginTop:10,borderLeft:'5px solid #ff9f1c'}}>
        <h3>LegalEstates + Rule7 PoA + Lost Logbook</h3>
        <p style={{fontSize:11}}>Will + Estate S18(3) &lt;R250k J243 + Lease + Affidavits DRAFT. &gt;R250k referral R750.</p>
        <p style={{fontSize:11,fontWeight:700}}>R99 Lite | R199 All | Capitec 2121467447</p>
      </div>

      <div style={{background:'white',color:'black',borderRadius:14,padding:14,marginTop:10,borderLeft:'5px solid #c1121f'}}>
        <h3>🔥 Bundles</h3>
        <p style={{fontSize:12}}>R299 Comply | R349 Family BEST | R399 Mega</p>
