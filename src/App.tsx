...
type Trip = {date:string; from:string; to:string; reason:string; o1:string; o2:string; type:string; km:number};

export default function App(){
  const [trips,setTrips]=useState<Trip[]>([]);
  const [form,setForm]=useState({date:'',from:'',to:'',reason:'',o1:'',o2:'',type:'Business'});
  const [r7,setR7]=useState({trav:'',subs:'',days:''});
  const [r7out,setR7out]=useState(''); const [biz,setBiz]=useState(0); const [priv,setPriv]=useState(0);
  const addTrip=()=>{
    const km=Number(form.o2)-Number(form.o1);
    if(km<=0){alert('Check odo');return;}
    const nt=[...trips,{...form,km} as Trip]; setTrips(nt);
    let b=0,p=0; nt.forEach(t=>{if(t.type==='Business')b+=t.km;else p+=t.km;}); setBiz(b); setPriv(p);
  };
  const genLB=()=>{
    if(!trips.length){alert('Add trips');return;}
    let txt=`SARS LOGBOOK IN17 DRAFT - Ilitha Fintech - Capitec 2121467447 - 0847392469\n${new Date().toLocaleString()}\n\n`;
    trips.forEach((x,i)=>{txt+=`${i+1}. ${x.date} ${x.from}->${x.to} ${x.reason} ${x.km}km ${x.type}\n`;});
    txt+=`\nBusiness ${biz}km Private ${priv}km Total ${biz+priv}km Pct ${biz+priv?Math.round(biz/(biz+priv)*100):0}%\nDRAFT - Keep 5 years`;
  ...
  const calcR7=()=>{
    ...
    setR7out(`Travel R${trav} Tax 20pct R${taxTrav.toFixed(2)} | Subs R${subs} Exempt R${exempt.toFixed(2)} Taxable R${taxSubs.toFixed(2)} - BGR41`);
  };
  return(
    <div style={{background:'#0a1930',color:'white',minHeight:'100vh',padding:12,fontFamily:'system-ui'}}>
      <div style={{background:'#fffbe6',color:'#000',border:'2px dashed #ff9f1c',padding:12,borderRadius:12,fontSize:12}}>
        <b>DEV RECOVERY - Capitec SL Cengcani 2121467447 - Branch 470010 - WhatsApp 0847392469</b><br/>Ref Cell+Package | POP to 0847392469 License ILITHA-30D-XXXX | Ilitha Fintech Reg 2026/707498/07
      </div>
      <h2 style={{marginTop:14}}>Ilitha Fintech Hub - Logbook IN17 + Rule7 + LegalEstates</h2>
      <div style={{background:'white',color:'black',borderRadius:14,padding:14,marginTop:12}}>
        <h3>ZATax + Logbook IN17 + Rule7 Calculator + BEE TEMPLATE</h3>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:8}}>
          ...
          <input placeholder="Reason" value={form.reason} onChange={e=>setForm({...form,reason:e.target.value})} style={{padding:8,border:'1px solid #ccc',borderRadius:8}}/>
        ...
        <p style={{fontSize:11,marginTop:6}}>Business {biz}km Private {priv}km Total {biz+priv}km</p>
        <button onClick={genLB} style={{width:'100%',padding:10,background:'#00a896',color:'white',borderRadius:20,fontWeight:700}}>Generate Logbook DRAFT</button>
        <div style={{marginTop:12}}>
          <h4>Rule7 Calculator Subsistence + Travel</h4>
          ...
          <div style={{fontSize:11,background:'#f0f8ff',padding:8,borderRadius:8,marginTop:6}}>{r7out||'Result here'}</div>
        </div>
      </div>
      <div style={{background:'#fff8e6',color:'black',borderRadius:14,padding:14,marginTop:10,borderLeft:'5px solid #ff9f1c'}}>
        <h3>LegalEstates + Rule7 PoA + Lost Logbook RC1</h3>
        <p style={{fontSize:11}}>Will Wills Act 2 witnesses + Estate S18(3) under R250k J243 Inventory TEMPLATE + Lease + Affidavits DRAFT. Over R250k referral R750.</p>
        <p style={{fontSize:11,fontWeight:700}}>R99 Lite | R199 All+Vault | Capitec 2121467447</p>
      </div>
      <div style={{background:'white',color:'black',borderRadius:14,padding:14,marginTop:10,borderLeft:'5px solid #c1121f'}}>
        <h3>Bundles R203k Target</h3>
        <p style={{fontSize:12}}>R299 Comply | R349 Family BEST | R399 Mega All 7</p>
        <a href="https://wa.me/27847392469?text=Paid%20Capitec%202121467447%20R349" style={{display:'inline-block',background:'#25D366',color:'white',padding:'10px 14px',borderRadius:20,textDecoration:'none',fontWeight:700}}>WhatsApp POP 0847392469</a>
      </div>
    </div>
  );
}
